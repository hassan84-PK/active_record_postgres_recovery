...
RSpec.describe ActiveRecordPostgresRecovery::SidekiqMiddleware do
  ...
  let(:job) { { 'jid' => 'jid-123' } }
  let(:queue) { 'default' }
  let(:clear_action) { { strategy: 'all', performed: true, roles: %i[writing reading], skipped_reason: nil } }

  before do
    allow(ActiveRecordPostgresRecovery::Handler).to receive(:report_attempted_recovery)
    allow(ActiveRecordPostgresRecovery::Handler).to receive(:clear_all_connections!).and_return(clear_action)
  end

  it 'clears all configured connections and re-raises matching db connectivity errors' do
  ...
  it 'does not clear connections for unrelated errors' do
    ...
    expect(ActiveRecordPostgresRecovery::Handler).not_to have_received(:clear_all_connections!)
  end
end
